...
<template>
  <div class="search">
    ...
    <div v-if="foundEvents" class="search-list">
      <div v-for="event in foundEvents" :key="event.id" class="search-details">
        <router-link :to="'/event/' + event.id">
          ...
          <div v-if="startDate" class="search-date">
            <p>start date search period: {{ startDate }}</p>
            <p>end date search period: {{ endDate }}</p>
          </div>
...
export default defineComponent({
  setup() {
    ...
    const getSearchResult = async () => {
      try {
        if (searchCategory.value != '' || status.value != '') {
          await fetch(`https://eonet.sci.gsfc.nasa.gov/api/v3/events?limit=15&status=${status.value}&category=${searchCategory.value}&start=${startDate.value}&end=${endDate.value}`)
